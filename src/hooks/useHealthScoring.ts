...
function useClientHealthScores(engagementIds: string[]) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Get engagements with client company details
      const { data: engagements, error } = await supabase
        ...
        .select(`
          ...
          framework:frameworks(id, name, health_thresholds)
        `)
        .in("id", engagementIds)
        .is("archived_at", null);
      ...
      const clientScores = await Promise.all(
        engagements.map(async (engagement) => {
          try {
            // Get client's active framework
            const { data: companyFramework } = await supabase
            ...
            const { data: metrics } = await supabase
              .from("framework_health_metrics")
              .select("*")
              .eq("framework_id", companyFramework.active_framework_id)
              .eq("enabled", true)
              .limit(5);
            ...
            for (const metric of metrics) {
              const calculator = metricCalculators[metric.calculation_key] || metricCalculators["default"];
              const { current, previous } = await calculator(engagement.client_company_id, 30);
              const weight = metric.weight_percent || (100 / metrics.length);
              totalScore += current * weight;
              totalWeight += weight;
              trendSum += current - previous;
            }

            const avgScore = totalWeight > 0 ? totalScore / totalWeight : 0;
            const rawThresholds = engagement.framework?.health_thresholds as Record<string, unknown> | null;
            ...
            return {
              ...
              status: avgScore >= thresholds.green ? "green" : avgScore >= thresholds.yellow ? "yellow" : "red",
              trend: trendSum > 5 ? "improving" : trendSum < -5 ? "declining" : "stable" as TrendDirection,
            };
          } catch {
            return {
              engagementId: engagement.id,
...
function useCoachingOrgHealthRollup() {
  ...
  return useQuery({
    queryKey: ["coaching-org-health-rollup", activeCompanyId],
    queryFn: async () => {
      if (!activeCompanyId) return null;

      // Get all engagements for this coaching org
      const { data: engagements, error } = await supabase
        .from("coaching_engagements")
        .select(`
          id,
      ...
      if (!engagements || engagements.length === 0) {
        return {
          totalClients: 0,
          distribution: { green: 0, yellow: 0, red: 0, unknown: 0 },
          trends: { improving: 0, stable: 0, declining: 0 },
          clients: [],
        };
      }

      // Calculate health for each client (simplified for performance)
      const clientHealths = await Promise.all(
        engagements.slice(0, 20).map(async (engagement) => {
          try {
            const { data: companyFramework } = await supabase
              .from("company_frameworks")
              .select("active_framework_id")
            ...
            if (!companyFramework) {
              return { status: "unknown" as const, trend: "stable" as TrendDirection, name: engagement.client?.name };
            }

            // Quick task-based health check
            ...
            const rate = totalTasks && totalTasks > 0 ? ((completedTasks || 0) / totalTasks) * 100 : 75;
            
            return {
              status: rate >= 80 ? "green" : rate >= 50 ? "yellow" : "red",
              trend: "stable" as TrendDirection,
      ...
      const distribution = {
        green: clientHealths.filter(c => c.status === "green").length,
        yellow: clientHealths.filter(c => c.status === "yellow").length,
        red: clientHealths.filter(c => c.status === "red").length,
        unknown: clientHealths.filter(c => c.status === "unknown").length,
      };

      const trends = {
        improving: clientHealths.filter(c => c.trend === "improving").length,
        stable: clientHealths.filter(c => c.trend === "stable").length,
        declining: clientHealths.filter(c => c.trend === "declining").length,
      };

      return {
        totalClients: engagements.length,
        distribution,
        trends,
